...
Empirically we find that the latent space is approximately linearly separable by gender according to multiple different orthogonal planes. So we just repeat the process: we learn the first gender probe $W_1$, calculate its nullspace $N(W\_1)$ and the projection $P\_{N(W\_1)}$ onto the nullspace, project the data to get a first “debiased” version $P\_{N(W_1)}X$, and then train the second gender classifier $W_2$, its nullsapce $N(W_2)$ and projection P\_N(W2), apply it to get a second “debiased” version of the data $P\_{N(W\_2)}P\_{N(W_1)}X$, and so forth. We continue this process until no linear probe achieve above random accuracy. At this point we return the final “debiasing” projection $P=P_n...P_2P_1$. This is the essence of our algorithm, which we call **Iterative Nullsapce Projection (INLP)**.
